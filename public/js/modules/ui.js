...
/* ── Sticky bar (removed from UI, kept as no-op so old calls don't crash) ── */
function hideStickyBar() {
  // No sticky bar in the current UI. This stub exists so legacy calls
  // elsewhere in this file don't throw ReferenceError.
}
...
export function initBikeCards() {
  ...
  // Autocomplete for all vehicle inputs
...
export function showError(msg) {
  ...
  el.innerHTML = `<strong>Oops! No vehicle found.</strong> ${msg} — Please wait a few minutes before you try again later!.`;
...
function buildReset(mode) {
  return `
    <div style="text-align:center;margin:24px 0 12px">
      <button class="btn btn-ghost" id="resetBtn">${mode === 'suggest' ? 'Try Another Budget' : mode === 'profile' ? 'Explore Another Vehicle' : 'Compare Again'}</button>
    </div>`;
}
