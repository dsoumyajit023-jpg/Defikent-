...
export function resetAll() {
  ...
  // Deselect all preference chips
  document.querySelectorAll('#prefChips .chip.active').forEach(chip => {
    chip.classList.remove('active');
  });

  // Clear custom pref
  const customPref = document.getElementById('customPref');
  if (customPref) customPref.value = '';

  // Reset sliders
  const km = document.getElementById('kmSlider');
  if (km) {
    km.value = CONFIG.KM_DEFAULT;
    document.getElementById('kmVal').textContent = `${CONFIG.KM_DEFAULT} km/day`;
  }
  const sal = document.getElementById('salarySlider');
  if (sal) {
    sal.value = CONFIG.SALARY_DEFAULT;
    document.getElementById('salaryVal').textContent = `₹${Number(CONFIG.SALARY_DEFAULT).toLocaleString('en-IN')}`;
  }
